...
// 静态文件服务中间件在所有路由的最前面，确保 CSS 和 JS 文件能被正确加载
...
apiRouter.get("/providers", (req, res) => {
  const providers = Object.values(apiPool).map(provider => ({
    id: provider.id,
    name: provider.name,
    type: provider.type,
  }));
...
apiRouter.post("/chat-stream", async (req, res) => {
  try {
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          ...
          try {
            ...
            } else if (type === "deepseek-reasoner") {
            ...
            if (thoughtChunk) {
              res.write(`data: ${JSON.stringify({ thought_chunk: thoughtChunk })}\n\n`);
            }
            if (chunkText) {
              fullReply += chunkText;
    ...
    res.write(`data: ${JSON.stringify({ done: true, fullReply: fullReply })}\n\n`);
  } catch (error) {
    console.error("[FATAL] 流式请求处理失败:", error.message);
